...
echo "start test (v4)" `date` >> $logfile
echo ./copy_progs -t -l $logfile
./copy_progs -t -p 1 -l $logfile
echo "Single NVM run, multiple DRAM runs"
for i in {1..20}
do
   echo ./copy_progs -n /mnt/pmem1/copy-test -p 1 -s 23 -a $i -l $logfile
   ./copy_progs -n /mnt/pmem1/copy-test -p 1 -s 23 -a $i -l $logfile
done
echo "Single DRAM run, multiple emulated NVM runs"
for i in {1..20}
do
   echo ./copy_progs -f /mnt/pmem1/copy-test -p 1 -s 23 -a $i -l $logfile
   ./copy_progs -f /mnt/pmem1/copy-test -p 1 -s 23 -a $i -l $logfile
done
...
for i in {1..20}
do
   echo ./copy_progs -n /mnt/pmem1/copy-test -p 1 -s 25 -a $i -l $logfile
   ./copy_progs -n /mnt/pmem1/copy-test -p 1 -s 25 -a $i -l $logfile
done
